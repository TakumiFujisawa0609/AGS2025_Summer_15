...
/// <param name="angleRadians">角度</param>
/// <returns></returns>
Vector2F AsoUtility::GetPointOnCircle(float centerX, float centerY,float radius, float angleRadians)
{
    Vector2F point;
    point.x = centerX + radius * cos(angleRadians);
    point.y = centerY + radius * sin(angleRadians);
...
/// <returns></returns>
std::vector<Vector2F> AsoUtility::GetPointsCircle(float centerX, float centerY, float radius, float angle, int numPoints)
{
    ...
    for (int i = 0; i < numPoints; i++) {
        ...
        points.push_back(GetPointOnCircle(centerX, centerY, radius, angle));
    }
...
/// <param name="a"></param>
/// <param name="p"></param>
void AsoUtility::DrawCirclePoints(float x, float y, float r,float a, int p)
{
    //円周上の点を計算
    std::vector<Vector2F>points = GetPointsCircle(x, y, r, a, p);

    //各点を描画
    for (int i = 0; i < (int)points.size(); i++) {
        DrawCircle(points[i].x, points[i].y, 4, 0xffffff);
    }
