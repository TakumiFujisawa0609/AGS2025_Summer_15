...
void Runboo::Update()
{
	...
	{
		if (!w->GetUnit().isAlive_) // éÄÇÒÇ≈Ç¢ÇÈÇ»ÇÁ
		{
			deadCount++;
		}
	}
...
void Runboo::Draw()
{
	...
	// ÉâÉìÉ_ÉÄîºåaÇ∆äpìxÇãÅÇﬂÇÈ
	float radius = 100.0f; // îºåa
	float angle = GetRand(359) * DX_PI_F / 180.0f;
	float dist = (float)GetRand((int)radius); // 0Å`îºåaÇ‹Ç≈ÇÃÉâÉìÉ_ÉÄãóó£

	// â~ì‡ÇÃÉâÉìÉ_ÉÄç¿ïWÇéZèo
