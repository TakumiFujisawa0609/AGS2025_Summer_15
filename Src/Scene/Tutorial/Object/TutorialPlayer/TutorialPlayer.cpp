...
void TutorialPlayer::Init()
{
	...
	//状態
	ChangeState(TutorialPlayer::STATE::MOVE);
...
void TutorialPlayer::Draw()
{
	if (unit_.isAlive_) {
		...
		//SetFontSize(30);
		//DrawFormatString(4, 734, 0xffffff, "%.2f,%.2f", unit_.disppos_.x, unit_.disppos_.y);
		//SetFontSize(16);
	}
}
...
// 状態遷移を一元管理する
void TutorialPlayer::StateManager(void)
{
	//各状態から遷移可能にさせたい状態の、遷移条件関数 ( DoState～～() ) を呼び出す
	switch (state_)
	{
	case TutorialPlayer::STATE::MOVE:
		DoStateAttack();
		DoStateEvasion();
		break;
	case TutorialPlayer::STATE::ATTACK:
		break;
	case TutorialPlayer::STATE::EVASION:
		break;
	case TutorialPlayer::STATE::DAMAGE:
		break;
	}
...
//移動状態
void TutorialPlayer::DoStateMove()
{
	auto& ins = KEY::GetIns();
	...
	if ((ins.GetInfo(KEY_TYPE::MOVE_UP).down) ||
		...
		(ins.GetInfo(KEY_TYPE::JUMP).down)) {
		ChangeState(TutorialPlayer::STATE::MOVE);
		return;
	}
...
// 攻撃状態
void TutorialPlayer::DoStateAttack()
{
	auto& ins = KEY::GetIns();
	...
	if (haveB_) {
		if (ins.GetInfo(KEY_TYPE::ATTACK).now) {
			ChangeState(TutorialPlayer::STATE::BP_ATTACK);
		...
		if (ins.GetInfo(KEY_TYPE::ATTACK).down == false) { return; }

		// 攻撃状態に遷移する
		ChangeState(TutorialPlayer::STATE::ATTACK);
...
// 回避状態
void TutorialPlayer::DoStateEvasion()
{
	...
	if (ins.GetInfo(KEY_TYPE::EVASION).down && evasionPossiFlg_) {
		ChangeState(TutorialPlayer::STATE::EVASION);
...
// 状態変更
void TutorialPlayer::ChangeState(STATE st)
{
	...
	{
	case TutorialPlayer::STATE::MOVE:
		state_ = TutorialPlayer::STATE::MOVE;
		stateFuncPtr = &TutorialPlayer::Move;
		break;
	case TutorialPlayer::STATE::ATTACK:
		state_ = TutorialPlayer::STATE::ATTACK;
		stateFuncPtr = &TutorialPlayer::Attack;
		break;
	case TutorialPlayer::STATE::BP_ATTACK:
		state_ = TutorialPlayer::STATE::BP_ATTACK;
		stateFuncPtr = &TutorialPlayer::BambooAttack;
		break;
	case TutorialPlayer::STATE::EVASION:
		state_ = TutorialPlayer::STATE::EVASION;
		stateFuncPtr = &TutorialPlayer::Evasion;
		break;
	case TutorialPlayer::STATE::DAMAGE:
		state_ = TutorialPlayer::STATE::DAMAGE;
		stateFuncPtr = &TutorialPlayer::Damage;
		break;
	}
}
...
// 攻撃状態
void TutorialPlayer::Attack()
{
	switch (attack_)
	{
	case TutorialPlayer::FIRST:
		// モーション更新
		ChangeMotion(MOTION::FIRST_ATTACK, false);
		break;
	case TutorialPlayer::SECONDE:
		// モーション更新
		ChangeMotion(MOTION::SECOND_ATTACK, false);
		break;
	case TutorialPlayer::THREE:
		// モーション更新
...
// 特殊攻撃状態
void TutorialPlayer::BambooAttack(void)
{
	auto& ins = KEY::GetIns();
...
// ダメージ処理
void TutorialPlayer::Damage(void)
{
	if (!knockBack_)ChangeState(TutorialPlayer::STATE::MOVE);

	ChangeMotion(TutorialPlayer::MOTION::DAMAGE);
...
void TutorialPlayer::Respawn(void)
{
...
void TutorialPlayer::IsGround(Collision::DIR dir)
{
	switch (dir)
	{
	case Collision::UP:
...
void TutorialPlayer::LoadPlayerImage(void)
{
...
void TutorialPlayer::ChangeMotion(MOTION mo, bool loop)
{
...
void TutorialPlayer::DrawPlayer(void)
{
	bool invic = false;
...
void TutorialPlayer::Hit(int damage, Vector2F bPos)
{
	if (state_ == TutorialPlayer::STATE::EVASION) {
		SceneManager::GetIns().HitStop();
		evaConpFlg_ = true;
		unit_.inviCounter_ = 50;
		return;
	}
	ChangeState(TutorialPlayer::STATE::DAMAGE);
