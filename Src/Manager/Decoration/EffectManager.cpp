...
/// <param name="_qua">角度</param>
/// <param name="_size">大きさ</param>
void EffectManager::Play2D(const EFFECT& _efc, const Vector2F& _pos, const float _angleZ, const float& _size)
{
	//元データがないときは警告
	if (effectRes_.find(_efc) == effectRes_.end())assert("設定していないエフェクトを再生しようとしています。");
	...
	if (effectPlay_.find(_efc) == effectPlay_.end()) {
		//入っていないとき要素を追加する
		effectPlay_.emplace(_efc, PlayEffekseer2DEffect(effectRes_[_efc]));
	}
	else {
		//入っていたら元あるやつに上書きする
		effectPlay_[_efc] = PlayEffekseer2DEffect(effectRes_[_efc]);
	}

	//各種設定同期
	Sync2DEffect(_efc, _pos, _angleZ, _size);
}
#pragma endregion

#pragma region 3Dエフェクトの停止
...
void EffectManager::Stop2D(const EFFECT& _efc)
{
	//配列内に入っていないものを停止しようとしたら警告
	if (effectPlay_.find(_efc) == effectPlay_.end())assert("設定していないエフェクトを停止しようとしています。");
	//再生停止
	StopEffekseer2DEffect(effectPlay_[_efc]);
}

#pragma region 3Dエフェクトの設定
...
void EffectManager::Sync2DEffect(const EFFECT& _efc, const Vector2F& _pos, const float _angleZ, const float& _size)
{
	//その他各種設定
	//大きさ
		//その他各種設定
	//大きさ
	SetScalePlayingEffekseer2DEffect(effectPlay_[_efc], _size, _size, 1);
	//角度
	SetRotationPlayingEffekseer2DEffect(effectPlay_[_efc],0,0,_angleZ);
	//位置
	//SetPosPlayingEffekseer2DEffect(effectPlay_[_efc], _pos.x, _pos.y,0);
	SetPosPlayingEffekseer2DEffect(effectPlay_[_efc], 600, 300,0);
...
bool EffectManager::IsPlay2DEffect(const EFFECT& _efc)
{
	if (effectPlay_[_efc] == -1 || IsEffekseer2DEffectPlaying(effectPlay_[_efc] == -1))
	{
		return true;
	}
	return false;
